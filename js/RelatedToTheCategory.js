...
let nameOfCategory =localStorage.getItem('nameOfCategory');
async function getDataOfMeals(nameOfCategory) {
    let response = await fetch (`https://www.themealdb.com/api/json/v1/1/filter.php?c=${nameOfCategory}`);
    let{meals} = await response.json();
    DisplayDataOfCategory (meals);
    setIdOfItem();
} 
function DisplayDataOfCategory (meals) {
    let cartoona='';
    meals.map((item)=>{
        cartoona+=`
